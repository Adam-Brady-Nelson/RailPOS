...
declare global {
  ...
  createOrderWithItems: (payload: { customerId: number; phoneId: number; fulfillment?: 'delivery' | 'collection' | 'bar' | 'restaurant'; items: Array<{ dish_id: number; quantity: number; price: number }>, payment_method?: 'cash' | 'card' }) => Promise<{ orderId: number }>
  getOrdersToday: () => Promise<Array<{ id: number; created_at: string; status: string; phone_id: number; fulfillment: 'delivery' | 'collection' | 'bar' | 'restaurant'; customer_name?: string; customer_phone?: string; total: number }>>
  ...
  order: { id:number; status:string; phone_id:number; payment_method: string | null; fulfillment: 'delivery' | 'collection' | 'bar' | 'restaurant'; created_at:string },
...
contextBridge.exposeInMainWorld('db', {
  ...
  createOrderWithItems: (payload: { customerId: number; phoneId: number; fulfillment?: 'delivery' | 'collection' | 'bar' | 'restaurant'; items: Array<{ dish_id: number; quantity: number; price: number }>, payment_method?: 'cash' | 'card' }) => ipcRenderer.invoke('create-order-with-items', payload),
